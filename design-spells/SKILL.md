...
3. **Simulated UIs (MANDATORY APPLIED RULES)**: 
   - **Bento Box Magic**: Design the features section using a Bento Box grid. Inside the cards, do not use static images. I want Simulated UI Micro-animations built with pure Tailwind CSS and SVGs. Build mini-interfaces (like fake cursors moving, typing effects, or sliding notification pop-ups) that automatically animate to explain the feature.
...
4. **Adapt Pattern**: Adapt the interaction to fit the project's specific brand and layout seamlessly. Use it to enhance the core narrative of the app.
5. **Implement flawlessly**: Use CSS, Anime.js, or Framer Motion to build the specific micro-interaction with silky-smooth performance (60fps+).
